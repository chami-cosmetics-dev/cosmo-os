/**
 * Restore main Dashboard for merchant-level roles; hide Merchant Dashboard perms.
 *
 * - ensure dashboard.view
 * - remove dashboard.merchant_view
 * - keep contacts.insight.read if present
 *
 * Usage:
 *   node scripts/with-env.mjs cosmo-prod node scripts/align-merchant-main-dashboard-perms.cjs
 */

const { PrismaClient } = require("@prisma/client");

const ROLE_NAMES = ["merchant-level-01", "merchant-level-02"];
const REMOVE_KEYS = ["dashboard.merchant_view"];
const ENSURE_KEYS = ["dashboard.view"];
...
async function main() {
  ...
  console.log(`[align-merchant-main-dash] dryRun=${dryRun}`);
  ...
  try {
    ...
    for (const roleName of ROLE_NAMES) {
      ...
      console.log(
        JSON.stringify({
          role: roleName,
          before: [...current].filter(
            (k) => k.startsWith("dashboard") || k.includes("insight")
          ),
          remove: toRemove,
          add: toAdd,
        })
