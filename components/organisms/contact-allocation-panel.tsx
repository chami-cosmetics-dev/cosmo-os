...
type ContactItem = {
  ...
  updatedAt: string;
  createdAt: string;
  // Allocation fields (populated from DB when available)
  remarks?: string | null;
  gender?: string | null;
...
export function ContactAllocationPanel({
  ...
  useEffect(() => {
    fetch("/api/admin/contacts/allocation/options")
      .then(async (res) => {
        if (!res.ok) return;
  ...
  async function handlePhoneSearch() {
    ...
    try {
      const res = await fetch(
        `/api/admin/contacts/allocation/lookup?phone=${encodeURIComponent(normalizedQuery)}`
      );
      const data = (await res.json()) as {
        found: boolean;
        contact?: ContactItem;
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/contacts/${selectedContact.id}/allocation`, {
        method: "PATCH",
