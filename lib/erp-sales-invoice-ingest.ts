...
export type ErpSalesInvoiceIngestCreds = {
  baseUrl: string;
  apiKey: string;
  apiSecret: string;
  label: string | null;
...
export type ErpSalesInvoiceIngestResult =
  | { ok: true; skipped: true; reason?: string }
  | { ok: true; returned: true; orderId?: string | null }
  | { ok: true; voided: true; orderId: string }
  | { ok: true; orderId: string; orderName: string | null; financialStatus: string }
...
export async function ingestParsedErpSalesInvoice(input: {
  data: ErpnextSalesInvoiceWebhookPayload;
  rawPayload: unknown;
  instanceCreds: ErpSalesInvoiceIngestCreds;
}): Promise<ErpSalesInvoiceIngestResult> {
  const { data, rawPayload, instanceCreds } = input;
  ...
  if (
    ...
    return { ok: true, skipped: true, reason: "credit_note" };
  }
  ...
  if (linkedVaultOrder) {
    ...
    return { ok: true, skipped: true, reason: "linked_vault_order" };
  }
