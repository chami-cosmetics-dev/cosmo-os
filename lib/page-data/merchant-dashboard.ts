...
export async function getMerchantDashboardPageData(input: {
  ...
  if (!input.viewerIsAdmin) {
    ...
    if (!merchants.some((m) => m.id === selectedMerchantId)) {
      return {
        error: "Your account does not have a merchant role (e.g. merchant-level-02)",
        status: 403,
      };
    }
  } else {
    ...
    if (!selectedMerchantId) {
      return {
        error: "No users with merchant roles found. Assign roles like merchant-level-02 or Merchant 01 first.",
        status: 404,
      };
    }
  }
