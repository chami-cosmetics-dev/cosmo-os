...
import {
  getGrnPendingEmailConfig,
  normalizeGrnEmailRecipientList,
  upsertGrnPendingEmailConfig,
...
export async function GET() {
  ...
  const config = await getGrnPendingEmailConfig(companyId);
  const lastSent = await prisma.grnPendingEmailSendLog.findFirst({
    where: { companyId },
  ...
  return NextResponse.json({
    ...
    recipients: normalizeGrnEmailRecipientList(config?.recipients),
...
export async function PUT(request: NextRequest) {
  ...
  const recipients = normalizeGrnEmailRecipientList(parsed.data.recipients);
  ...
  await upsertGrnPendingEmailConfig({
    companyId,
  ...
  await writeAuditLog({
    ...
    module: "settings",
    entityType: "GrnPendingEmailConfig",
    entityId: companyId,
    summary: `Updated GRN pending email settings (${recipients.length} recipient(s), enabled=${parsed.data.enabled})`,
