...
export async function GET(request: NextRequest) {
  ...
  // Always rank by last-30-days sales (high → low) before paginating
  const salesWindow = resolveAssistWindow({ asOfDate });
  const allSalesForRank =
    unique.length > 0
      ? await aggregateSalesBySkuInRange(
          companyId,
          salesWindow.rangeStart,
          salesWindow.rangeEndExclusive,
        )
      : new Map<string, number>();

  const salesByLower = new Map<string, number>();
  for (const [sku, qty] of allSalesForRank) {
    const key = sku.trim().toLowerCase();
    salesByLower.set(key, (salesByLower.get(key) ?? 0) + qty);
  }
  const salesFor = (sku: string) => salesByLower.get(sku.trim().toLowerCase()) ?? 0;

  unique = [...unique].sort((a, b) => {
    const sa = salesFor(a.sku!);
    const sb = salesFor(b.sku!);
    if (sb !== sa) return sb - sa;
    return (a.sku ?? "").localeCompare(b.sku ?? "");
  });

  // Top sales view: only SKUs with sales in the window (true top sellers)
  if (mode === "top_sales") {
    unique = unique.filter((row) => salesFor(row.sku!) > 0);
  }
