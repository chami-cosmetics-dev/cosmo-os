...
export default async function OgfLogsPage() {
  ...
  const logs = companyId
    ? await prisma.ogfEmailLog.findMany({
        where: { companyId },
  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-3xl border border-border/70 bg-[linear-gradient(135deg,var(--dashboard-hero-start),var(--dashboard-hero-middle),var(--dashboard-hero-end))] p-6 shadow-[0_18px_40px_-28px_var(--primary)]">
        ...
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">OGF Integration</p>
        <h1 className="mt-2 text-3xl font-semibold tracking-tight">OGF Email Logs</h1>
        <p className="mt-2 max-w-3xl text-sm text-muted-foreground sm:text-base">
          History of nightly sales sync emails sent to the OGF notification address. Each row is one batch — click Resend to re-send a failed or missing email.
        </p>
        {!isOgfConfigured && (
          <div className="mt-4 inline-flex rounded-lg border border-yellow-400/50 bg-yellow-50/80 px-4 py-2 text-sm text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300">
            OGF_LOCATION_ID is not configured — sync runs only on the Cosmo OS deployment.
          </div>
        )}
      </section>
