...
import { DailySalesSmsResendButton } from "./daily-sales-sms-resend-button";
      ...
      <Card className="overflow-hidden border-border/70 shadow-xs">
        <CardHeader className="border-b border-border/50">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-muted-foreground">
              {smsLogs.length} record{smsLogs.length !== 1 ? "s" : ""}
            </span>
          </div>
        </CardHeader>
        <CardContent className="p-0">
          {smsLogs.length === 0 ? (
            <div className="p-6 text-sm text-muted-foreground">
              No daily sales SMS attempts yet. Configure recipients under Settings → SMS Portal, then
              wait for the nightly job or use Send test / Resend.
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
